...
@interface HZPhotoBrowser() <UIScrollViewDelegate,UIActionSheetDelegate>
...
-(void)longPressAction:(UILongPressGestureRecognizer *)longPress
{
    ...
    //创建一个活动卡
    UIActionSheet * action_Sheet = [[UIActionSheet alloc]initWithTitle:nil delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:@"举报" otherButtonTitles:nil,nil];
   
    action_Sheet.actionSheetStyle = UIActionSheetStyleBlackTranslucent;
    //设置哪一个索引对应的按钮为红色高亮显示
    ...
    return;
    HClActionSheet * actionSheet = [[HClActionSheet alloc] initWithTitle:nil style:HClSheetStyleWeiChat itemTitles:@[@"举报"]];
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex;
{
    ...
    NSLog(@"clickedButtonAtIndex %@",[actionSheet buttonTitleAtIndex:buttonIndex]);
    if (buttonIndex == 0) {
        NSLog(@"分享到朋友圈");
       ...
       // - (void)hidePhotoBrowser:(UITapGestureRecognizer *)recognizer
        ...
        //发送通知
        [[NSNotificationCenter defaultCenter] postNotificationName:@"didjubaoObjNotification" object:_model.id];
