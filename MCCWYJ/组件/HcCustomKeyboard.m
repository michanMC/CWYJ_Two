...
-(void)textViewShowView:(UIViewController *)viewController customKeyboardDelegate:(id)delegate
{
    ...
    self.mBackView =[[UIView alloc]initWithFrame:CGRectMake(0, [UIScreen mainScreen].bounds.size.height, WIDTH_SCREEN, 50)];
    ...
    self.mBackView.backgroundColor = [UIColor whiteColor];
    _mBackView.layer.borderColor = [UIColor grayColor].CGColor;
    _mBackView.layer.borderWidth= 1;
   ...
   // self.mSecondaryBackView.backgroundColor =[UIColor colorWithRed:153/255.0 green:154/255.0 blue:158/255.0 alpha:1];
    ...
    self.mTextView =[[UITextView alloc]initWithFrame:CGRectMake(1, 1, Main_Screen_Width - 35 * 2 - 10 - 5, 28)];
   ...
   // _mTalkBtn.backgroundColor = [UIColor yellowColor];
    [self.mTalkBtn addTarget:self action:@selector(forTalk) forControlEvents:UIControlEventTouchUpInside];
    self.mTalkBtn.frame =CGRectMake(Main_Screen_Width - 35 - 5, 10, 35, 25);
    [self.mTalkBtn setTintColor:[UIColor blackColor]];
    [self.mBackView addSubview:self.mTalkBtn];
    ViewRadius(_mTalkBtn, 5);
    _mTalkBtn.layer.borderColor = [UIColor lightGrayColor].CGColor;
    _mTalkBtn.layer.borderWidth = 1.0;
    _mTalkBtn.titleLabel.font = AppFont;
    
    _zuozheBtn = [[UIButton alloc]initWithFrame:CGRectMake(Main_Screen_Width - 5 - 35-35 - 5, 10, 35, 25)];
   // _zuozheBtn.backgroundColor = [UIColor yellowColor];
   // ViewRadius(_zuozheBtn, 25/2);
    ViewRadius(_zuozheBtn, 5);

    _zuozheBtn.layer.borderColor = [UIColor lightGrayColor].CGColor;
    _zuozheBtn.layer.borderWidth = 1.0;
    [_zuozheBtn setTitleColor:[UIColor grayColor] forState:0];
    [self.mBackView addSubview:self.zuozheBtn];
    [_zuozheBtn setTitleColor:[UIColor orangeColor] forState:UIControlStateSelected];
    [self.zuozheBtn setTitle:@"@作者" forState:UIControlStateNormal];
    _zuozheBtn.titleLabel.font = [UIFont systemFontOfSize:11];
...
{    
    self.isTop =NO;
        [UIView animateWithDuration:0.3 animations:^{
        self.mBackView.frame=CGRectMake(0, Main_Screen_Height, WIDTH_SCREEN, 50);
        self.mHiddeView.alpha =0.0f;
