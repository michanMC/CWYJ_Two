...
//        needShowNotification = (dict && [dict valueForKey:RedpacketKeyRedpacketTakenMessageSign]) ? NO : needShowNotification;
        ...
        if (needShowNotification) {
            [MCUserDefaults setObject:@"newMessage" forKey:@"1"];
            //发送通知
...
//#if !TARGET_IPHONE_SIMULATOR
            ...
            switch (state) {
                case UIApplicationStateActive://前台
                {
                    [MCIucencyView playSoundAndVibration];
                }
                    break;
                case UIApplicationStateInactive://后台
                    [MCIucencyView playSoundAndVibration];
                    break;
                case UIApplicationStateBackground://后台
...
//        
        ...
        if (!isAppActivity) {//在后台
            ...
            notification.alertBody = aMessage;//[NSString stringWithFormat:NSLocalizedString(@"friend.somebodyAddWithName", @"%@ add you as a friend"), aUsername];
            //设置userinfo 方便在之后需要撤销的时候使用
            
            NSDictionary *infoDic = [NSDictionary dictionaryWithObject:@"addFriend" forKey:@"key"];
...
//            notification.alertAction = NSLocalizedString(@"open", @"Open");
    ...
    NSLog(@"加红点");
    [MCIucencyView playSoundAndVibration];
    [MCUserDefaults setObject:@"1" forKey:@"addfriend"];
