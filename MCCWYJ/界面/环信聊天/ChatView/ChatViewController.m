                           ...
                           modelForMessage:(EMMessage *)message
{
    ...
    model.avatarImage = [UIImage imageNamed:@"home_Avatar_44"];
    NSLog(@"什么鬼1");
    NSLog(@"model ========%@",model);
    NSLog(@"model.isSender= %zd",model.isSender);
    
    if (model.isSender) {
        
        
        model.avatarURLPath = [MCUserDefaults objectForKey:@"thumbnail"];
        model.nickname = [MCUserDefaults objectForKey:@"nickname"];
    ...
    {
        NSString * nickname = @"";
        if (self.useModel.nickname.length) {
            nickname = self.useModel.nickname;
        }
        else if(self.useModel.mobile.length){
            nickname =  [CommonUtil phonenumbel:self.useModel.mobile];
        ...
        model.avatarURLPath = self.useModel.thumbnail;
        model.nickname = nickname;
        NSLog(@"_useModel.nickname == %@",nickname);

    }
    model.failImageName = @"city-card_default-photo";
    return model;
}
...
- (BOOL)isEmotion2MessageFormessageViewController:(EaseMessageViewController *)viewController
                                    messageModel:(id<IMessageModel>)messageModel
{
    BOOL flag = NO;
    
    if ([messageModel.message.ext objectForKey:@"is_card_call"] ) {
        return YES;
    }
    return flag;
}
