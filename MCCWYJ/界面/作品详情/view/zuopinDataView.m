...
-(UIView*)addHeadView:(NSInteger)indexCount{
   ...
   // NSDictionary * dicimg = [NSDictionary dictionary] ;
    ...
    if (indexCount == 1) {
        ...
        UIButton * imgView =[self addImgView:imgfram ImgStr:@"buy_default-photo" ImgUrlStr:photomodel.raw];
        imgView.tag = 400;
    ...
    BOOL isjing = [_home_model.isAuslese boolValue];
    ...
    _jinView = [[UIImageView alloc]initWithFrame:CGRectMake(bgview.mj_w - 5 - 30, bgview.mj_h - 5 - 40, 30, 40)];
    _jinView.image = [UIImage imageNamed:@"水印"];
    if( [_home_model.collectCount integerValue]>= [ausleseCount integerValue]){
        _jinView.hidden = NO;
//        isjing = YES;
...
//    [imgView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",imgUrlStr]] forState:0 placeholderImage:[UIImage imageNamed:@"矢量智能对象"]];
   ...
   // imgView2.image = [UIImage imageNamed:@"buy_default-photo"];
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section==0) {
      ...
      cell.isgengduan = _isjiagengduo  ;
        ...
        [cell.headimgBtn addTarget:self action:@selector(actionHeaderBtn) forControlEvents:UIControlEventTouchUpInside];
        //姓名
        ...
        NSLog(@"======%d",_home_model.collection);
...
//        //游记类型
//        NSLog(@">>>>%@",[self.classifyDic objectForKey:[NSString stringWithFormat:@"%ld",_home_model.classify]]);
        _classifyDic = @{
                         @"0":@"食1",
                         @"1":@"住1",
                         @"2":@"景1",
        ...
        cell.keyImgStr = [_classifyDic objectForKey:[NSString stringWithFormat:@"%ld",_home_model.classify]] ;
...
-(void)loadModle:(BOOL)iszhuan{
   ...
   // [self showLoading:iszhuan AndText:nil];
        ...
        [self.requestManager postWithUrl:@"api/travel/detail.json" refreshCache:NO params:Parameterdic IsNeedlogin:YES success:^(id resultDic) {
            NSLog(@"成功");
            NSLog(@"返回==%@",resultDic);
...
-(void)actionShouchang:(UIButton*)btn{
    ...
    [self.requestManager postWithUrl:collection refreshCache:NO params:Parameterdic IsNeedlogin:YES success:^(id resultDic) {
        [_deleGate stop:nil];
        NSLog(@"成功");
        [self loadModle:YES];
        //发送通知
...
-(void)actionheaderBtn:(UIButton*)btn{
    ...
    pinglunModel * model = _dataPingLunArray[btn.tag - 450];
    homeYJModel * model2 = [[homeYJModel alloc]init];
    model2.userModel =model.userModel;
    //发送通知
    [[NSNotificationCenter defaultCenter] postNotificationName:@"didCarteObjNotification" object:model2];
