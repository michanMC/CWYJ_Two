...
-(void)loadData{
    ...
    NSDictionary * dic = @{
                           @"page":@(_pageStr),
                           @"destUid":[MCUserDefaults objectForKey:@"id"],
                    

                           };
    [self.requestManager postWithUrl:@"api/travel/myTravels.json" refreshCache:NO params:dic IsNeedlogin:YES success:^(id resultDic) {
...
//    if (_dataAarray.count > indexPath.section) {
    ...
    else
    {
       ...
       cell.selectBtn.selected = model.isdelete;
       cell.selectBtn.tag = indexPath.section + 500;
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_editView.hidden) {
        homeYJModel * homemodel = _dataAarray[indexPath.section];
...
-(void)actionCellSeleBtn:(UIButton*)btn{
    homeYJModel * model = _dataAarray[btn.tag - 500];
    if (btn.selected) {
        btn.selected = NO;
        model.isdelete = NO;
...
-(void)actionAllSeleBtn:(UIButton*)btn{
    ...
    if (_allSelectBtn.selected) {
        ...
        for (homeYJModel * model in _dataAarray) {
            model.isdelete = NO;
        }
...
-(void)actiondeleteBtn:(UIButton*)btn{
    ...
    NSDictionary * Parameterdic = @{
                                    @"travelIds":collectionIds
                                    };
    
    [self showLoading];
    [self.requestManager postWithUrl:@"api/travel/delete.json" refreshCache:NO params:Parameterdic IsNeedlogin:YES success:^(id resultDic) {
        [self stopshowLoading];
        NSLog(@"成功");
        NSLog(@"返回==%@",resultDic);
        [_seleArray removeAllObjects];
