...
@interface BillCollectViewController ()<UITableViewDelegate,UITableViewDataSource,UIAlertViewDelegate>
...
@implementation BillCollectViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    //监听数据的刷新
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(RefreshHeader) name:@"didBillCollectObjNotification" object:nil];
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (_isNoData) {
        ...
        [cell prepareNoDataUI:self.view.mj_h TitleStr:@"暂时没有收藏数据，点击去收藏"];
...
-(void)actiondeleteBtn:(UIButton*)btn{
    ...
    NSDictionary * Parameterdic = @{
                                    @"buyIds":collectionIds
                                    };
    
    [self showLoading];
    [self.requestManager postWithUrl:@"api/buy/cancelCollect.json" refreshCache:NO params:Parameterdic IsNeedlogin:YES success:^(id resultDic) {
        [self stopshowLoading];
        NSLog(@"成功");
        NSLog(@"返回==%@",resultDic);
        [_seleArray removeAllObjects];
...
-(void)actionTapBtn{
    _delegate.tabBarController.selectedIndex = 1;
