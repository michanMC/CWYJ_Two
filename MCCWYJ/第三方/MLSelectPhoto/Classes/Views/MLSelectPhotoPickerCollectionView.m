    ...
    if(indexPath.item == 0 && self.topShowPhotoPicker){
        ...
        if (self.isRecoderSelectPicker) {
            for (MLSelectPhotoAssets *asset in self.doneAsstes) {
                ...
                MLSelectPhotoAssets * ass = self.dataArray[indexPath.item];
                
                if ([asset isKindOfClass:[MLSelectPhotoAssets class]] && [asset.asset.defaultRepresentation.url isEqual:[ass asset].defaultRepresentation.url]) {
