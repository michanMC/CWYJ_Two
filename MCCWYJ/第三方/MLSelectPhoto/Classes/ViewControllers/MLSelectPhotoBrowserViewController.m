...
-(void)delete{
    ...
    for (MLSelectPhotoAssets *asset in _photos) {
        [arr addObject:asset];
    }
    
    if (arr.count > _currentPage) {
        [arr removeObjectAtIndex:_currentPage];
        if (_currentPage >0)
        _currentPage --;
        //- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
//        MLSelectPhotoPickerBrowserPhotoScrollView *scrollView = [self.view viewWithTag:500];
//        [self scrollViewDidScroll:scrollView];
        
       [self setPageLabelPage:_currentPage];
//        - (void)setPageLabelPage:(NSInteger)page{

        [_delgetView deletearray:arr];
        _photos =arr;
...
- (void)viewDidLoad {
    [super viewDidLoad];
    if (_isliulan) 
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc]initWithImage:[UIImage imageNamed:@"SelectTheDeliveryAddress_Delete_nor"] style:UIBarButtonItemStylePlain target:self action:@selector(delete)];
...
- (void)viewWillDisappear:(BOOL)animated{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:PICKER_REFRESH_DONE object:nil userInfo:@{@"assets":self.doneAssets?self.doneAssets:@""}];
